...
public class Optimization {
    ...
    public <V extends DataType<V>> int argmin(
            ...
            MultiObjectiveIEPOSAgent agent) {
        ...
        sb.append("    | [Proposal Evaluation] Agent " + agent.getPeer().getIndexNumber() + " (Iter " + agent.getIteration() + ")\n");
        
        String globalCostName = costFunction.getLabel();
        String localCostName = agent.getLocalCostFunction().getLabel();
        sb.append(String.format("    | Cost Functions: Global='%s', Local='%s'\n", globalCostName, localCostName));
        
        int numChildren = agent.getChildren().size();
        sb.append("    | Evaluating " + responseCombos.size() + " proposals (combinations of children's responses):\n");
        sb.append("    | (Generated from " + numChildren + " children x 2 options each = " + (int)Math.pow(2, numChildren) + " combinations)\n");
        sb.append("    | [Reasoning]: Each child sends 2 options (Keep Old Plan vs. Switch to New Plan).\n");
        sb.append("    |              We must check every combination to see which set of choices\n");
        sb.append("    |              results in the lowest Global Cost.\n");

        for(int i = 0; i < responseCombos.size(); i++) {
            sb.append("    |   [Proposal " + i + "]\n");
            V combined;
            ...
            sb.append("    |     Global Cost Calculation:\n");
            costFunction.logCostCalculation(combined, sb);
            double cost = costFunction.calcCost(combined);
            sb.append(String.format("    |     => Global Cost = %.4f\n", cost));
        ...
        util.NarrativeWriter.write(sb.toString());

        return this.extendedOptimization(costs, alpha, beta, discomfortSums, discomfortSumSqrs, numAgents, agent, "Proposal Selection Optimization");
    }
    ...
    public <V extends DataType<V>> int argmin(
        ...
        StringBuilder sb = new StringBuilder();
        sb.append("    ___________________________________________________________________\n");
        sb.append("    | [Plan Cost Calculation] Agent " + agent.getPeer().getIndexNumber() + " (Iter " + agent.getIteration() + ")\n");
        
        String globalCostName = costFunction.getLabel();
        String localCostName = localCostFunction.getLabel();
        sb.append(String.format("    | Cost Functions: Global='%s', Local='%s'\n", globalCostName, localCostName));
        
        sb.append("    | Calculating costs for " + choices.size() + " candidate plans:\n");

        for (int i = 0; i < choices.size(); i++) {
            sb.append("    |   [Candidate Plan " + i + "]\n");
            ...
            // Log Local Cost
            sb.append("    |     Local Cost Calculation:\n");
            localCostFunction.logCostCalculation(choices.get(i), sb);
            double score = localCostFunction.calcCost(choices.get(i));
            sb.append(String.format("    |     => Local Cost = %.4f\n", score));
...
//                    double[] sumArray = valueOfConstraintViolated(response, true, Math.min(length, response.length));
                    ...
                    if (sum > 0) {
                        ...
                        sb.append(String.format("    |     [Hard Constraint Violation] Cost penalized to %.4f\n", cost));
                    }
                }
            }
...
//        }
        ...
        return this.extendedOptimization(costs, alpha, beta, discomfortSums, discomfortSumSqrs, numAgents, agent, "Plan Selection Optimization");
    ...
    private <V extends DataType<V>> int extendedOptimization(double[] costs,				double alpha,
                                                             ...
                                                             MultiObjectiveIEPOSAgent agent, String context) {

        double minCost = Double.POSITIVE_INFINITY;
        int selected = -1;
        int numOpt = 0;
        
        String itemName = context.contains("Proposal") ? "Proposal" : "Plan";
        
        StringBuilder sb = new StringBuilder();
        sb.append("    ___________________________________________________________________\n");
        sb.append("    | [" + context + "] Agent " + agent.getPeer().getIndexNumber() + " (Iter " + agent.getIteration() + ")\n");
        
        String globalCostName = agent.getGlobalCostFunction().getLabel();
        String localCostName = agent.getLocalCostFunction().getLabel();
        sb.append(String.format("    | Parameters: Alpha=%.4f, Beta=%.4f\n", alpha, beta));
        sb.append(String.format("    | Cost Functions: Global='%s', Local='%s'\n", globalCostName, localCostName));
        
        sb.append("    | Comparing " + costs.length + " possible " + itemName.toLowerCase() + "s:\n");

        try {
            for(int i = 0; i < costs.length; i++) {
                double cost = costs[i];
                double originalGlobalCost = cost;
                double weightedCost = cost;
                ...
                if(alpha > 0 || beta > 0) {
                    ...
                    double localCost = PlanSelectionOptimizationFunctionCollection.localCost(discomfortSums[i], numAgents);
                    double unfairness = PlanSelectionOptimizationFunctionCollection.unfairness(discomfortSums[i], discomfortSumSqrs[i], numAgents);
                    
                    sb.append(String.format("    |   %s %d: GlobalCost=%.4f\n", itemName, i, originalGlobalCost));
                    sb.append(String.format("    |           Formula: Cost = (1-a-b)*Global + beta*Local + alpha*Unfairness\n"));
                    sb.append(String.format("    |           Components:\n"));
                    sb.append(String.format("    |             - Global Cost: %.4f\n", originalGlobalCost));
                    sb.append(String.format("    |             - Local Cost:  %.4f (Sum=%.2f / N=%.0f)\n", localCost, discomfortSums[i], numAgents));
                    sb.append(String.format("    |             - Unfairness:  %.4f (StdDev of Local Costs)\n", unfairness));
                    sb.append(String.format("    |           Calculation:\n"));
                    sb.append(String.format("    |             (%.2f * %.4f) + (%.2f * %.4f) + (%.2f * %.4f)\n", 
                            (1-alpha-beta), originalGlobalCost, beta, localCost, alpha, unfairness));
                    sb.append(String.format("    |           => WeightedCost = %.4f\n", weightedCost));
                } else {
                    sb.append(String.format("    |   %s %d: GlobalCost=%.4f (Alpha=0, Beta=0)\n", itemName, i, originalGlobalCost));
                }
            ...
            sb.append(String.format("    | => Selected %s %d with Min Cost %.4f\n", itemName, selected, minCost));
            sb.append("    |__________________________________________________________________\n");
            util.NarrativeWriter.write(sb.toString());
