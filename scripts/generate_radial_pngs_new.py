#!/usr/bin/env python3
"""
generate_radial_pngs_new.py

Generates per-iteration radial tree PNGs (no table) for BOTH local_cost and 
complex_cost color modes, plus a separate CSV table per iteration.

Output structure:
  <run_dir>/radial_visualisation_new/
    tree_local_cost_iter_000.png
    tree_complex_cost_iter_000.png
    table_iter_000.csv
    ...
"""
import re
import argparse
import os
import sys
import matplotlib.pyplot as plt
import networkx as nx
import pandas as pd
import numpy as np
from matplotlib.patches import Circle, Rectangle
from matplotlib import cm
from matplotlib.colors import LinearSegmentedColormap, Normalize

# --- Configuration ---
COLORS_RED    = ['#ffffb2', '#fed976', '#feb24c', '#fd8d3c', '#fc4e2a', '#e31a1c', '#b10026']
COLORS_PURPLE = ['#fcfbfd', '#efedf5', '#dadaeb', '#bcbddc', '#9e9ac8', '#807dba', '#6a51a3', '#4a1486']

def get_text_color_for_bg(rgba_color):
    """
    Determine if text should be white or black based on background color luminance.
    """
    if isinstance(rgba_color, (tuple, list)) and len(rgba_color) >= 3:
        r, g, b = rgba_color[0], rgba_color[1], rgba_color[2]
    else:
        return 'black'
    
    def adjust_channel(c):
        if c <= 0.03928:
            return c / 12.92
        else:
            return ((c + 0.055) / 1.055) ** 2.4
    
    r_lin = adjust_channel(r)
    g_lin = adjust_channel(g)
    b_lin = adjust_channel(b)
    
    luminance = 0.2126 * r_lin + 0.7152 * g_lin + 0.0722 * b_lin
    return 'white' if luminance < 0.5 else 'black'

def parse_log_for_topology(log_path):
    run_data = {}
    
    current_run = 0
    current_iter_phase = None
    current_agent_phase = None
    
    calc_agent = None
    calc_iter = None
    calc_plan = None
    cost_cache = {}
    
    re_sim = re.compile(r'=== Simulation (\d+) ===')
    re_calc_header = re.compile(r'\[Plan Cost Calculation\] Agent (\d+) \(Iter (\d+)\)')
    re_cand_plan = re.compile(r'\s+\[Candidate Plan (\d+)\]')
    re_cand_gcost = re.compile(r'\s+=> Global Cost = ([\d\.]+)')
    re_iter_phase = re.compile(r'\[Iter (\d+)\] \[BOTTOM-UP PHASE\] Agent (\d+):')
    re_parent = re.compile(r'- Sending aggregated proposal to Parent \(Agent (\d+)\)')
    re_plan_id = re.compile(r'- I selected Plan ID: (\d+) \(Cost: ([\d\.]+)\)')

    if not os.path.exists(log_path):
        return {}

    with open(log_path, 'r', encoding='utf-8', errors='replace') as f:
        for line in f:
            line = line.strip()
            if not line: continue
            
            m_sim = re_sim.search(line)
            if m_sim:
                current_run = int(m_sim.group(1)) - 1
                continue
            
            m_calc = re_calc_header.search(line)
            if m_calc:
                calc_agent = int(m_calc.group(1))
                calc_iter = int(m_calc.group(2))
                calc_plan = None
                continue
            
            if calc_agent is not None:
                m_cp = re_cand_plan.search(line)
                if m_cp:
                    calc_plan = int(m_cp.group(1))
                    continue
                m_cgc = re_cand_gcost.search(line)
                if m_cgc and calc_plan is not None:
                    gcost = float(m_cgc.group(1))
                    cost_cache[(current_run, calc_iter, calc_agent, calc_plan)] = gcost
                    continue
                if line.startswith('[') and 'Candidate Plan' not in line:
                    calc_agent = None
            
            m_iter = re_iter_phase.search(line)
            if m_iter:
                current_iter_phase = int(m_iter.group(1))
                current_agent_phase = int(m_iter.group(2))
                
                if current_run not in run_data: run_data[current_run] = {}
                if current_iter_phase not in run_data[current_run]:
                     run_data[current_run][current_iter_phase] = {
                         'edges': [], 'selected_plans': {},
                         'local_costs': {}, 'complex_costs': {}
                     }
                continue
            
            if current_agent_phase is not None and current_iter_phase is not None:
                m_p = re_plan_id.search(line)
                if m_p:
                    pid = int(m_p.group(1))
                    lcost = float(m_p.group(2))
                    
                    iter_d = run_data[current_run][current_iter_phase]
                    iter_d['selected_plans'][current_agent_phase] = pid
                    iter_d['local_costs'][current_agent_phase] = lcost
                    
                    cc = cost_cache.get((current_run, current_iter_phase, current_agent_phase, pid))
                    if cc is not None:
                        iter_d['complex_costs'][current_agent_phase] = cc
                
                m_par = re_parent.search(line)
                if m_par:
                    parent_id = int(m_par.group(1))
                    run_data[current_run][current_iter_phase]['edges'].append((current_agent_phase, parent_id))
                    
    return run_data

def parse_conf_for_numchildren(run_dir):
    num_children = 2
    conf_path = os.path.join(run_dir, 'used_conf.txt')
    if os.path.exists(conf_path):
        with open(conf_path, 'r', encoding='utf-8') as f:
            for line in f:
                line = line.strip()
                if line.startswith('numChildren ='):
                    try:
                        num_children = int(line.split('=')[1].strip())
                    except ValueError:
                        pass
    return num_children

def get_ternary_radial_pos(G, root, node_radius=0.3):
    from collections import deque
    
    parent_map = {root: None}
    levels = {root: 0}
    queue = deque([root])
    
    while queue:
        node = queue.popleft()
        for neighbor in G.neighbors(node):
            if neighbor not in levels:
                levels[neighbor] = levels[node] + 1
                parent_map[neighbor] = node
                queue.append(neighbor)
                
    max_depth = max(levels.values()) if levels else 1
    
    min_arc_spacing = node_radius * 4
    max_nodes = 3 ** max_depth
    min_radius_for_crowded = max_nodes * min_arc_spacing / (2 * np.pi)
    base_radius = max(min_radius_for_crowded / max_depth, node_radius * 5)
    
    pos = {}
    
    def assign_positions(node, center_angle, angular_width, depth):
        if depth == 0:
            pos[node] = np.array([0.0, 0.0])
        else:
            r = depth * base_radius
            x = r * np.cos(center_angle)
            y = r * np.sin(center_angle)
            pos[node] = np.array([x, y])
            
        children = [n for n in G.neighbors(node) if parent_map.get(n) == node]
        if not children:
            return
            
        children.sort()
        
        if depth == 0:
            piece_width = 2 * np.pi / 3
            start_angle = 0
            for i, child in enumerate(children):
                assign_positions(child, start_angle + i * piece_width, piece_width, depth + 1)
        else:
            piece_width = angular_width / 3
            start_angle = center_angle - angular_width / 2 + piece_width / 2
            
            for i, child in enumerate(children):
                # Spread up to 3 children around center
                assign_positions(child, start_angle + i * piece_width, piece_width, depth + 1)
                
    assign_positions(root, 0, 2 * np.pi, 0)
    return pos

def get_radial_pos(G, root, node_radius=0.3):
    """
    Generate a circular radial tree layout with NO edge crossings.
    Root at center. Each subtree gets a contiguous angular wedge.
    All nodes at same depth are on the same circle (same radius).
    """
    from collections import deque
    
    parent_map = {root: None}
    levels = {root: 0}
    queue = deque([root])
    
    while queue:
        node = queue.popleft()
        for neighbor in G.neighbors(node):
            if neighbor not in levels:
                levels[neighbor] = levels[node] + 1
                parent_map[neighbor] = node
                queue.append(neighbor)
    
    level_nodes = {}
    for node, level in levels.items():
        if level not in level_nodes:
            level_nodes[level] = []
        level_nodes[level].append(node)
    
    def count_leaves(node):
        children = [n for n in G.neighbors(node) if parent_map.get(n) == node]
        if not children:
            return 1
        return sum(count_leaves(c) for c in children)
    
    leaf_counts = {node: count_leaves(node) for node in G.nodes()}
    
    max_depth = max(levels.values()) if levels else 1
    max_nodes_at_level = max(len(nodes) for nodes in level_nodes.values())
    min_arc_spacing = node_radius * 4
    min_radius_for_crowded = max_nodes_at_level * min_arc_spacing / (2 * np.pi)
    base_radius = max(min_radius_for_crowded / max_depth, node_radius * 5)
    
    pos = {}
    
    def assign_positions(node, angle_start, angle_end, depth):
        if depth == 0:
            pos[node] = np.array([0.0, 0.0])
        else:
            mid_angle = (angle_start + angle_end) / 2
            r = depth * base_radius
            x = r * np.cos(mid_angle)
            y = r * np.sin(mid_angle)
            pos[node] = np.array([x, y])
        
        children = [n for n in G.neighbors(node) if parent_map.get(n) == node]
        if not children:
            return
        
        children.sort()
        total_child_leaves = sum(leaf_counts[c] for c in children)
        angle_range = angle_end - angle_start
        current_angle = angle_start
        
        for child in children:
            child_leaves = leaf_counts[child]
            child_angle_range = angle_range * child_leaves / total_child_leaves
            assign_positions(child, current_angle, current_angle + child_angle_range, depth + 1)
            current_angle += child_angle_range
    
    assign_positions(root, 0, 2 * np.pi, 0)
    return pos


def render_tree(ax, G, pos, root, node_ids, node_colors, plan_changed,
                node_radius, font_size, title_text):
    """Render the radial tree on the given axes (no table, no slider)."""
    ax.set_aspect('equal')
    ax.axis('off')
    
    # Draw edges
    nx.draw_networkx_edges(G, pos, ax=ax, arrowstyle='-', edge_color='#aaaaaa', alpha=0.5, width=1.5)
    
    # Draw nodes
    for idx, n in enumerate(node_ids):
        is_root = (n == root)
        border_color = '#00ff00' if is_root else 'white'
        
        # Double border if plan changed
        if plan_changed[idx]:
            outer_circle = Circle(pos[n], radius=node_radius * 1.3, facecolor='none',
                                  edgecolor=border_color, linestyle='solid', linewidth=2.5, zorder=9)
            ax.add_patch(outer_circle)
        
        circle = Circle(pos[n], radius=node_radius, facecolor=node_colors[idx],
                        edgecolor=border_color, linestyle='solid',
                        linewidth=2.5, zorder=10)
        ax.add_patch(circle)
        
        text_color = get_text_color_for_bg(node_colors[idx])
        ax.text(pos[n][0], pos[n][1], str(n), ha='center', va='center',
                fontsize=font_size, weight='bold', zorder=11, color=text_color)
    
    ax.autoscale_view()
    xlim = ax.get_xlim()
    ylim = ax.get_ylim()
    margin = node_radius * 2
    ax.set_xlim(xlim[0] - margin, xlim[1] + margin)
    ax.set_ylim(ylim[0] - margin, ylim[1] + margin)
    
    ax.set_title(title_text, fontsize=14, weight='bold', pad=20, color='white')


def render_slider(ax, gc_values, curr_gc):
    """Render the global cost slider bar."""
    ax.axis('off')
    if not gc_values:
        return
    
    ax.set_xlim(-0.5, len(gc_values) - 0.5)
    ax.set_ylim(0, 1)
    ax.plot([0, len(gc_values) - 1], [0.5, 0.5], color='#666666', linewidth=6, zorder=1)
    
    for idx_gc, gc_val in enumerate(gc_values):
        is_current = abs(gc_val - curr_gc) < 1e-6
        
        marker_color = '#fc4e2a' if is_current else '#888888'
        marker_size = 18 if is_current else 10
        ax.plot(idx_gc, 0.5, 'o', color=marker_color, markersize=marker_size, zorder=3)
        
        font_weight = 'bold' if is_current else 'normal'
        fs = 12 if is_current else 9
        fc = 'white' if is_current else '#aaaaaa'
        ax.text(idx_gc, 0.1, f"{gc_val:.2f}", ha='center', va='top',
                fontsize=fs, weight=font_weight, color=fc)
    
    ax.text((len(gc_values) - 1) / 2, 0.9, "Global Cost", ha='center', va='bottom',
            fontsize=12, weight='bold', color='white')


def create_visualization(run_dir, run_idx, output_folder):
    log_path = os.path.join(run_dir, 'algorithm_log.txt')
    run_data = parse_log_for_topology(log_path)
    num_children = parse_conf_for_numchildren(run_dir)
    
    if run_idx not in run_data:
        print(f"Run {run_idx} not found in log parsing results (or log missing).")
        print("The algorithm_log.txt file is required for tree topology.")
        return
    
    iters = sorted(run_data[run_idx].keys())
    
    # Read Global Cost CSV
    gc_df = None
    gc_path = os.path.join(run_dir, 'global-cost.csv')
    if os.path.exists(gc_path):
        gc_df = pd.read_csv(gc_path)
    
    # Global Min/Max Local Cost
    all_lcosts = []
    for it in iters:
        all_lcosts.extend(run_data[run_idx][it]['local_costs'].values())
    min_lc = min(all_lcosts) if all_lcosts else 0
    max_lc = max(all_lcosts) if all_lcosts else 1

    # Global Min/Max Complex Cost
    all_ccosts = []
    for it in iters:
        all_ccosts.extend(run_data[run_idx][it]['complex_costs'].values())
    min_cc = min(all_ccosts) if all_ccosts else 0
    max_cc = max(all_ccosts) if all_ccosts else 1
    
    # Global costs for slider
    all_global_costs = {}
    if gc_df is not None:
        col = f'Run-{run_idx}'
        if col in gc_df.columns:
            for _, row in gc_df.iterrows():
                all_global_costs[int(row['Iteration'])] = row[col]
    gc_values = sorted(set(all_global_costs.values()))
    
    # Colormaps
    cmap_lc = LinearSegmentedColormap.from_list("custom_red", COLORS_RED, N=256)
    norm_lc = Normalize(vmin=min_lc, vmax=max_lc)
    
    cmap_cc = LinearSegmentedColormap.from_list("custom_purple", COLORS_PURPLE, N=256)
    norm_cc = Normalize(vmin=min_cc, vmax=max_cc)
    
    os.makedirs(output_folder, exist_ok=True)
    
    prev_plans = {}
    
    for i, it in enumerate(iters):
        print(f"Generating Iteration {it}...")
        
        data = run_data[run_idx][it]
        edges = data['edges']
        
        # Build Graph
        G = nx.DiGraph()
        nodes = set(data['selected_plans'].keys())
        for u, v in edges:
            nodes.add(u)
            nodes.add(v)
            G.add_edge(v, u)
        G.add_nodes_from(nodes)
        
        roots = [n for n, d in G.in_degree() if d == 0]
        root = roots[0] if roots else (list(nodes)[0] if nodes else 0)
        
        n_agents = len(nodes)
        node_radius = max(0.15, min(0.4, 8.0 / n_agents))
        font_size = max(7, min(12, int(node_radius * 30)))
        
        if num_children == 3:
            pos = get_ternary_radial_pos(G, root, node_radius=node_radius)
        else:
            pos = get_radial_pos(G, root, node_radius=node_radius)
        
        all_x = [pos[n][0] for n in pos]
        all_y = [pos[n][1] for n in pos]
        extent = max(max(all_x) - min(all_x), max(all_y) - min(all_y)) if all_x else 10
        fig_size = max(12, extent * 1.2)
        
        curr_gc = all_global_costs.get(it, 0.0)
        
        # --- Compute plan changes (shared between both trees) ---
        node_ids = list(G.nodes())
        plan_changed = []
        for n in node_ids:
            pid = data['selected_plans'].get(n)
            prev_pid = prev_plans.get(n)
            if prev_pid is not None and pid == prev_pid:
                plan_changed.append(False)
            else:
                plan_changed.append(True)
        
        # --- 1. Tree colored by LOCAL COST ---
        node_colors_lc = []
        for n in node_ids:
            val = data['local_costs'].get(n, min_lc)
            node_colors_lc.append(cmap_lc(norm_lc(val)))
        
        fig_bg_color = (13/255, 15/255, 26/255, 0.96)
        fig = plt.figure(figsize=(fig_size, fig_size + 2), facecolor=fig_bg_color)
        gs = fig.add_gridspec(2, 1, height_ratios=[10, 1], hspace=0.1)
        
        ax_tree = fig.add_subplot(gs[0, 0])
        render_tree(ax_tree, G, pos, root, node_ids, node_colors_lc, plan_changed,
                    node_radius, font_size,
                    f"Iteration {it} | Global Cost: {curr_gc:.4f} | Color: Local Cost")
        
        ax_slider = fig.add_subplot(gs[1, 0])
        render_slider(ax_slider, gc_values, curr_gc)
        
        lc_file = os.path.join(output_folder, f"tree_local_cost_iter_{it:03d}.png")
        fig.savefig(lc_file, dpi=150, bbox_inches='tight', facecolor=fig_bg_color)
        plt.close(fig)
        print(f"  -> {lc_file}")
        
        # --- 2. Tree colored by COMPLEX COST ---
        node_colors_cc = []
        for n in node_ids:
            val = data['complex_costs'].get(n, min_cc)
            node_colors_cc.append(cmap_cc(norm_cc(val)))
        
        fig = plt.figure(figsize=(fig_size, fig_size + 2), facecolor=fig_bg_color)
        gs = fig.add_gridspec(2, 1, height_ratios=[10, 1], hspace=0.1)
        
        ax_tree = fig.add_subplot(gs[0, 0])
        render_tree(ax_tree, G, pos, root, node_ids, node_colors_cc, plan_changed,
                    node_radius, font_size,
                    f"Iteration {it} | Global Cost: {curr_gc:.4f} | Color: Complex Cost")
        
        ax_slider = fig.add_subplot(gs[1, 0])
        render_slider(ax_slider, gc_values, curr_gc)
        
        cc_file = os.path.join(output_folder, f"tree_complex_cost_iter_{it:03d}.png")
        fig.savefig(cc_file, dpi=150, bbox_inches='tight', facecolor=fig_bg_color)
        plt.close(fig)
        print(f"  -> {cc_file}")
        
        # --- 3. Separate TABLE CSV ---
        complex_costs = data.get('complex_costs', {})
        sorted_agents = sorted(list(nodes))
        
        table_rows = []
        for ag in sorted_agents:
            pid = data['selected_plans'].get(ag, '-')
            lc = data['local_costs'].get(ag, 0.0)
            cc = complex_costs.get(ag, 0.0)
            lc_str = f"{lc:.4f}" if isinstance(lc, float) else str(lc)
            cc_str = f"{cc:.4f}" if isinstance(cc, float) else str(cc)
            table_rows.append({'Agent': ag, 'Plan': pid, 'LocalCost': lc_str, 'ComplexCost': cc_str})
        
        table_df = pd.DataFrame(table_rows)
        table_file = os.path.join(output_folder, f"table_iter_{it:03d}.csv")
        table_df.to_csv(table_file, index=False)
        print(f"  -> {table_file}")
        
        # Update prev plans
        for n in node_ids:
            pid = data['selected_plans'].get(n)
            if pid is not None:
                prev_plans[n] = pid


def main():
    parser = argparse.ArgumentParser(
        description="Generate radial tree PNGs (both cost colors) + table CSV per iteration."
    )
    parser.add_argument('run_dir', help="Path to run output folder")
    parser.add_argument('--run', type=int, default=0)
    args = parser.parse_args()
    
    out_folder = os.path.join(args.run_dir, 'radial_visualisation_new')
    create_visualization(args.run_dir, args.run, out_folder)
    print(f"Done. Visualizations saved to {out_folder}")

if __name__ == "__main__":
    main()
